...
class AmassScan(ExternalProgramTask):
    ...
    def program_args(self):
        ...
        if self.exempt_list:
            command.append("-blf") # Path to file providing blacklisted subdomains
...
class ParseAmassOutput(luigi.Task):
    ...
    def output(self):
        ...
        return {
            ...
            "target-ips6": luigi.LocalTarget(f"{self.target_file}.ip6s"),
            "target-subdomains": luigi.LocalTarget(f"{self.target_file}.subdomains"),
            "target-asns": luigi.LocalTarget(f"{self.target_file}.asns") 
            }
    ...
    def run(self):
        ...
        ip6_file = self.output().get("target-ips6").open("w")
        subdomain_file = self.output().get("target-subdomains").open("w")
        asn_file = self.output().get("target-asns").open("w")

        with amass_json as aj, ip_file as ip_out, ip6_file as ip6_out, subdomain_file as subdomain_out, asn_file as asn_out:
            for line in aj:
